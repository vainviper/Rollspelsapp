...
ArmorConstruction.prototype.createDropdown = function () {
    for (let i=0;i<this.layer.length;i++) {  
        for(let j=0;j<this.areaCover.length;j++) {
            ...
            this.para.href ="#";
            this.para.classList="clickProof " + "layer" + this.layer[i];
            let id = this.armorId;
            this.para.onclick = function (id) {writeMaterials(id);};
            this.location = this.areaCover[j] + "_drop_" + this.layer[i] + "_drop";
...
function writeArmorSet (armorSetArray) {
       clearAllArmor();
       for (let i=0; i<armorSetArray.length;i+=2) {
                let writeArmorLayer = armorSetArray[i+1];
                let writeArmorId = armorSetArray[i];
        ...
        let stickDamage = 0;
            for (let j=1;j<5;j++) { //j<5 equals numbers of layers you can write menues to.
                ...
                if (krossDamage === 0 && huggDamage === 0 && stickDamage === 0) {
                        ...
                        paintArea(chooseBodyPartId,"peachpuff");
                } else {              
...
// FUNCTIONS FOR OPENING THE MENUES AND MOUSEEVENTS

function menuDrop (id) {
    let id_part = document.getElementById(id).id;
    dropdown(id_part);
};
function dropdown (id) {
    let idString = id + "_drop";
    document.getElementById(idString).classList.toggle("show");
};

addEventListener('click', whatIsClicked);
addEventListener('mouseover', whatIsItHoveringOn);
addEventListener('mouseout', whatIsItHoveringOn); 

function whatIsItHoveringOn (event) { //continue
...
function whatIsClicked (event) {
        ...
        let targetArea = clickedArea + "_drop";
        let targetAreaSpecs = document.getElementById(targetArea);
        if (!event.target.matches('.clickProof')) {
                let dropdowns = document.getElementsByClassName("dropdown-content");
                closeAllWindows(event, dropdowns);
        } else if (event.target.matches('.newAreaMenu')) {
                if (targetAreaSpecs.matches('.show')) { 
                        menuDrop(clickedArea);
                } else {
                        let dropdowns = document.getElementsByClassName("mainMenu");
                        closeAllWindows(event, dropdowns);
                        menuDrop(clickedArea);
                }     
        } else if (event.target.matches('.subMenu1a')) {
                if (targetAreaSpecs.matches('.show')) { 
                        menuDrop(clickedArea);
                } else {
                let dropdowns = document.getElementsByClassName("subMenu1b");
                closeAllWindows(event, dropdowns);
                menuDrop(clickedArea);
                }
        } else if (event.target.matches('.subMenu2a')) {
                if (targetAreaSpecs.matches('.show')) { 
                        menuDrop(clickedArea);
                } else {
                let dropdowns = document.getElementsByClassName("subMenu2b");
                closeAllWindows(event, dropdowns);
                menuDrop(clickedArea);
                }
        }
...
function closeAllWindows (event, dropdowns) {
